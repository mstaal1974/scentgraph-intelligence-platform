...
CREATE TABLE enrichment_reviews (
  id BIGSERIAL PRIMARY KEY,
  match_candidate_id BIGINT NOT NULL UNIQUE REFERENCES match_candidates(id) ON DELETE RESTRICT,
...
CREATE TABLE profile_drafts (
  ...
  CHECK (review_status <> 'approved_for_catalogue' OR (reviewer IS NOT NULL AND approved_at IS NOT NULL AND restricted_content_detected = FALSE AND source_confidence >= 0.700)),
